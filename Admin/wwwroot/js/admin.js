...
window.uploadFile = async (inputElement, endpoint) => {
    ...
    try {
        ...
        window.showToast('Hata', 'Dosya yükleme başarısız oldu.', 'error');
        return null;
    }
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    textareas.forEach(textarea => {
        ...
        window.autoResizeTextarea(textarea); // Initial resize
    });
});
...
window.copyToClipboard = async (text) => {
    try {
        ...
        window.showToast('Başarılı', 'Panoya kopyalandı.', 'success');
        return true;
    } catch (err) {
        console.error('Copy failed:', err);
        window.showToast('Hata', 'Panoya kopyalama başarısız oldu.', 'error');
        return false;
    }
};
